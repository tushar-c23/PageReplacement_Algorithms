...
void plotGraph(vector<double> x, vector<double> y, char* flag, int maxFrameSize, int maxPageSize)
{
    ...
    wstring titleText = L"Most Recently Used-" + flagText + L"-" + to_wstring(maxFrameSize) + L"-" + to_wstring(maxPageSize);
    string titleTextString = "Most Recently Used-" + flagTextString + "-" + to_string(maxFrameSize) + "-" + to_string(maxPageSize);
